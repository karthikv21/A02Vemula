...
QUnit.test('Testing compute Function with several inputs', function (assert) {
    assert.equal(App.compute(243,10), 24, 'Tested with two random positive integers');
    assert.equal(App.compute(-1,-7), 0,'Tested with two random negative integers which returns 0');
    assert.equal(App.compute("hey","hi"), 0,'Tested with string inputs which are not numbers which returns 0');  
    assert.equal(App.compute("9,-8"),0,'Tested with one positive and one negative number which returns 0');
